        ...
        public async Task<IActionResult> GetAllChildrenAsync()
        {
            try
            {
                ...
                for (int i = 0; i < children.Count(); i++)
                {
                    ...
                    if (grade.Count == 0)
                    {
                        childrenResponse[i].GradeLevelID = 0;
                        childrenResponse[i].GradeLevelName = string.Empty;
                        continue;
                    }
        ...
        public async Task<IActionResult> GetChildrenByClassIdAsync(int classId)
        {
            try
            {
                ...
                for (int i = 0; i < response.Count; i++)
                {
                    ...
                    response[i].ParentName = parent.FullName;
                    response[i].PhoneNumber = parent.PhoneNumber;
                    //Gan ApplicationID cho ChildResponse
